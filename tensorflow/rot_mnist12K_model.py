...
def single_branch(x, number_of_filters, number_of_fc_features):
  ...
  with tf.variable_scope('conv2'):
    max_pool2 = conv_relu_maxpool(max_pool1,
                                  [3, 3, number_of_filters, 2 * number_of_filters],
                                  [2 * number_of_filters])
  with tf.variable_scope('conv3'):
    max_pool3 = conv_relu_maxpool(max_pool2,
                                  [3, 3, 2 * number_of_filters, 4 * number_of_filters],
                                  [4 * number_of_filters])
...
'''x are batches nx32x32x1xnumber_of_rotations'''
def define_model(x,
                 keep_prob,
                 number_of_classes,
                 number_of_filters,
  ...
  with tf.variable_scope('fc2'):
    logits = fc(drop, [number_of_fc_features, number_of_classes], [number_of_classes])
  return logits
