...
train_step = tf.train.AdamOptimizer(1e-4).minimize(cross_entropy) #rho=0.9, epsilon=1e-06 AdamOptimizer(1e-5) AdadeltaOptimizer(learning_rate=1e-4, rho=0.9, epsilon=1e-06)
...
while (True):
  ...
  if (train_data_loader.is_new_epoch()):
    ...
    if (train_data_loader.get_completed_epochs() % 10 == 0):
      ...
      # all_x, all_y = test_data_loader.all()
      # test_accuracy = session.run(accuracy, feed_dict={x : all_x,
      #                                           y_gt : all_y,
      #                                           keep_prob : 1.0})
